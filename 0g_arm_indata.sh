...
function install_node() {
    ...
    # 配置节点
    wget https://raw.githubusercontent.com/a3165458/0g.ai/main/gensis.json -O $EVMSD_HOME/.evmosd/config/genesis.json
    ...
    sed -i -e "s/^seeds *=.*/seeds = \"$SEEDS\"/; s/^persistent_peers *=.*/persistent_peers = \"$PEERS\"/" $EVMSD_HOME/.evmosd/config/config.toml

    # 设置gas
    sed -i "s/^minimum-gas-prices *=.*/minimum-gas-prices = \"0.00252aevmos\"/" $EVMSD_HOME/.evmosd/config/app.toml

    # 设置用户端口
    sed -i -e "s%^proxy_app = \"tcp://127.0.0.1:26658\"%proxy_app = \"tcp://127.0.0.1:13458\"%; s%^laddr = \"tcp://127.0.0.1:26657\"%laddr = \"tcp://127.0.0.1:13457\"%; s%^pprof_laddr = \"localhost:6060\"%pprof_laddr = \"localhost:13460\"%; s%^laddr = \"tcp://0.0.0.0:26656\"%laddr = \"tcp://0.0.0.0:13456\"%; s%^prometheus_listen_addr = \":26660\"%prometheus_listen_addr = \":13466\"%" $EVMSD_HOME/.evmosd/config/config.toml
    sed -i -e "s%^address = \"tcp://localhost:1317\"%address = \"tcp://0.0.0.0:13417\"%; s%^address = \":8080\"%address = \":13480\"%; s%^address = \"localhost:9090\"%address = \"0.0.0.0:13490\"%; s%^address = \"localhost:9091\"%address = \"0.0.0.0:13491\"%; s%:8545%:13445%; s%:8546%:13446%; s%:6065%:13465%" $EVMSD_HOME/.evmosd/config/app.toml
    ...
    # 备份当前的验证者状态文件
    cp $EVMSD_HOME/.evmosd/data/priv_validator_state.json $EVMSD_HOME/.evmosd/priv_validator_state.json.backup

    # 重置数据目录同时保留地址簿
    evmosd tendermint unsafe-reset-all --home $EVMSD_HOME/.evmosd --keep-addr-book

    # 将快照解压直接到 .evmosd 目录
    lz4 -d -c ./latest_snapshot.tar.lz4 | tar -xf - -C $EVMSD_HOME/.evmosd

    # 恢复验证者状态文件的备份
    mv $EVMSD_HOME/.evmosd/priv_validator_state.json.backup $EVMSD_HOME/.evmosd/data/priv_validator_state.json
...
function uninstall_node() {
    ...
    case "$response" in
        [yY][eE][sS]|[yY])
            ...
            rm -rf $EVMSD_HOME/.evmosd $EVMSD_HOME/evmos $(which evmosd) && rm -rf 0g-evmos
            echo "节点程序卸载完成。"
            ;;
        *)
            echo "取消卸载操作。"
